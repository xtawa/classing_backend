...
function handleFab() {
  if (state.view === "aiAdmin") { document.getElementById("aiConfigForm")?.requestSubmit(); return; }
...
async function renderMembership() {
  ...
  const usage = usageResponse.usage || {}; const creditBalance = Number(usage.creditBalance || 0);
  ...
  content.innerHTML = `<div class="runtime-page">${hero("membership", `<div class="hero-stat"><strong>${item.isMember ? escapeHTML(item.tier) : "FREE"}</strong><span>${item.isMember ? `有效至 ${formatDate(item.expiresAt)}` : "当前方案"}</span></div>`)}
    <div class="runtime-grid"><section class="runtime-panel half"><h2>当前权益</h2><div class="empty-state"><span class="status-pill ${item.isMember ? "" : "warn"}">${item.isMember ? "会员有效" : "免费账户"}</span><strong>${item.isMember ? escapeHTML(item.tier) : "每月 500 点免费 AI 额度"}</strong><span>${item.isMember ? `到期时间：${formatDate(item.expiresAt, true)}` : "免费额度每月重置；会员过期后，已购永久额度会冻结至续费。"}</span></div></section>
    ...
    ${item.isMember ? `<section class="runtime-panel full"><div class="runtime-panel-header"><div><h2>购买 AI 算力额度</h2><p class="form-hint">仅限有效会员购买；额度进入永久余额，不随月额度重置。</p></div><span class="status-pill">永久余额 ${creditBalance.toLocaleString("zh-CN")} 点</span></div><div class="payment-layout ai-credit-purchase"><div class="payment-qr-placeholder" role="img" aria-label="AI 算力收款码位置预留"><strong>收款码</strong><span>位置预留</span></div><div class="payment-copy"><strong>2 元可购买 2,500 算力点</strong><p>支付后将包含支付单号的截图发送至邮箱。核验后，额度会直接增加到当前账户，并在会员月额度用完后自动使用。</p><a class="tonal-button purchase-mail-link" href="${escapeHTML(purchaseMailto)}">发送支付截图邮件</a><small>收件邮箱：zeromostia@gmail.com</small></div></div></section>` : `<section class="runtime-panel full"><div class="runtime-panel-header"><div><h2>AI 永久额度</h2><p class="form-hint">AI 额度仅限有效会员购买和使用。</p></div><span class="status-pill warn">已冻结 ${creditBalance.toLocaleString("zh-CN")} 点</span></div><div class="empty-state"><strong>会员过期，永久额度已冻结</strong><span>续费会员后将自动恢复，不会清零；免费账户仍可使用每月 500 点免费额度。</span></div></section>`}</div></div>`;
...
async function renderAskAI() {
  ...
  const creditBalance = Number(usage.creditBalance || 0); const creditAvailable = Number(usage.creditAvailable || 0);
  const consumed = usage.limit < 0 ? 0 : Math.min(usage.limit, usage.used + usage.reserved);
  const remaining = usage.limit < 0 ? "不限" : Math.max(0, usage.limit + creditAvailable - usage.used - usage.reserved).toLocaleString("zh-CN");
  setFab("新建对话");
...
function updateAIQuotaMeter(usage) {
  ...
  const credit = Number(usage.creditBalance || 0); const availableCredit = Number(usage.creditAvailable || 0); const remaining = usage.limit < 0 ? "不限" : Math.max(0, usage.limit + availableCredit - usage.used - usage.reserved).toLocaleString("zh-CN");
  ...
  meter.querySelector("p").textContent = usage.limit < 0 ? "当前账户不受算力额度限制" : `总可用 ${remaining} 点；永久余额 ${credit.toLocaleString("zh-CN")} 点${usage.creditFrozen ? "（会员过期，已冻结）" : ""}；月额度将在 ${formatDate(usage.resetAt, true)} 重置`;
}
...
function aiAdminUsageRow(row) {
  ...
  const quota = limit < 0
    ...
    : `<strong>${limit.toLocaleString("zh-CN")} 点</strong><small>${row.mode === "INHERIT" ? (row.isMember ? "继承会员默认值" : "免费账户默认值") : "用户级设置"}</small>`;
  ...
  return `<tr><td><strong>${escapeHTML(row.username)}</strong><br><small>${escapeHTML(row.email)} · ${row.isMember ? "会员" : "免费"}</small></td><td>${escapeHTML(row.period)}</td><td>${used.toLocaleString("zh-CN")}</td><td>${reserved.toLocaleString("zh-CN")}</td><td class="quota-value">${quota}</td><td>${progress}</td><td><strong>${credit.toLocaleString("zh-CN")} 点</strong><br><small>${row.creditFrozen ? "会员过期 · 已冻结" : "会员可用"}</small></td><td>${escapeHTML(aiQuotaModeLabel(row.mode))}</td><td><div class="runtime-actions"><button class="tonal-button" type="button" data-set-ai-quota="${escapeHTML(row.userId)}">月额度</button><button class="tonal-button" type="button" data-grant-ai-credit="${escapeHTML(row.userId)}">追加</button></div></td></tr>`;
}
...
async function renderAIAdmin() {
  ...
  content.innerHTML = `<div class="runtime-page">${hero("aiAdmin")}
    <div class="runtime-grid"><section class="runtime-panel full"><div class="runtime-panel-header"><h2>提供商与 Prompt</h2><span class="status-pill">${item.secretConfigured ? "密钥已配置" : "密钥缺失"}</span></div>
      <form class="runtime-form" id="aiConfigForm"><label class="form-field"><span>启用</span><select name="enabled"><option value="0" ${!item.enabled ? "selected" : ""}>关闭</option><option value="1" ${item.enabled ? "selected" : ""}>启用</option></select></label><label class="form-field"><span>默认模型</span><select name="model"><option value="deepseek-v4-flash" ${item.model !== "deepseek-v4-pro" ? "selected" : ""}>DeepSeek V4 Flash</option><option value="deepseek-v4-pro" ${item.model === "deepseek-v4-pro" ? "selected" : ""}>DeepSeek V4 Pro</option></select></label><label class="form-field full"><span>Base URL</span><input name="baseUrl" value="${escapeHTML(item.baseUrl)}" placeholder="启用 AI 时必填，例如 https://provider.example/v1"></label><label class="form-field"><span>环境变量密钥引用</span><input name="secretRef" value="${escapeHTML(item.secretRef)}" placeholder="启用 AI 时必填，例如 AI_PROVIDER_KEY_DEFAULT"></label><label class="form-field"><span>会员默认每月算力点</span><input name="defaultMonthlyLimit" type="number" min="0" value="${Number(item.defaultMonthlyLimit)}" required><small>免费账户固定为每月 500 点</small></label><label class="form-field"><span>最大输出 tokens</span><input name="maxOutputTokens" type="number" min="1" max="8192" value="${Number(item.maxOutputTokens)}" required><small>范围：1–8,192</small></label><label class="form-field"><span>超时（秒）</span><input name="timeoutSeconds" type="number" min="5" max="180" value="${Number(item.timeoutSeconds)}" required><small>范围：5–180 秒</small></label><label class="form-field"><span>历史消息上限</span><input name="maxHistoryMessages" type="number" min="2" max="200" value="${Number(item.maxHistoryMessages)}" required><small>范围：2–200 条</small></label><label class="form-field full"><span>System Prompt</span><textarea name="systemPrompt">${escapeHTML(item.systemPrompt)}</textarea></label><label class="form-field full"><span>课表 Prompt</span><textarea name="timetablePrompt">${escapeHTML(item.timetablePrompt)}</textarea></label><div class="runtime-actions full"><button class="primary-button">保存配置</button></div></form>
    ...
    <section class="runtime-panel full"><div class="runtime-panel-header"><div><h2>批量用户算力额度</h2><p class="form-hint">继承模式下，免费账户每月 500 点，会员使用上方会员默认值；管理员仍可单独覆盖。</p></div><span class="status-pill">会员默认 ${Number(item.defaultMonthlyLimit).toLocaleString("zh-CN")} 点 / 月</span></div><form class="runtime-form" id="aiQuotaForm"><label class="form-field full"><span>用户 ID、用户名或邮箱（逗号分隔）</span><input name="userIds" required></label><label class="form-field"><span>额度模式</span><select name="mode"><option value="INHERIT">继承默认额度</option><option value="LIMITED">自定义额度</option><option value="UNLIMITED">不限量</option><option value="BLOCKED">禁用 Ask AI</option></select></label><label class="form-field"><span>自定义每月算力点</span><input name="monthlyLimit" type="number" min="1" value="${Math.max(1, Number(item.defaultMonthlyLimit) || 10000)}"></label><div class="runtime-actions full"><button class="primary-button">应用算力额度</button></div></form></section>
    <section class="runtime-panel full" id="aiCreditGrantPanel"><div class="runtime-panel-header"><div><h2>追加永久算力额度</h2><p class="form-hint">适用于会员购买核验或补偿。非会员收到的额度会冻结，续费后自动恢复。</p></div><span class="status-pill">2 元 / 2,500 点</span></div><form class="runtime-form" id="aiCreditGrantForm"><label class="form-field"><span>用户 ID、用户名或邮箱</span><input name="userId" required></label><label class="form-field"><span>追加算力点</span><input name="points" type="number" min="1" max="100000000" value="2500" required></label><label class="form-field full"><span>备注</span><input name="note" maxlength="240" placeholder="例如：支付单号或补偿原因"></label><div class="runtime-actions full"><button class="primary-button">确认追加永久额度</button></div></form></section></div></div>`;
  document.getElementById("aiConfigForm").addEventListener("submit", async (event) => { event.preventDefault(); const button = event.submitter || event.currentTarget.querySelector("button"); await runButtonAction(button, async () => { try { const data = Object.fromEntries(new FormData(event.currentTarget)); await api("/api/v1/admin/ai/config", { method:"PUT", body:JSON.stringify({ enabled:Number(data.enabled), providerKind:"OPENAI_COMPATIBLE", baseUrl:data.baseUrl.trim(), model:data.model, secretRef:data.secretRef.trim(), systemPrompt:data.systemPrompt, timetablePrompt:data.timetablePrompt, temperature:0.2, maxOutputTokens:Number(data.maxOutputTokens), timeoutSeconds:Number(data.timeoutSeconds), maxHistoryMessages:Number(data.maxHistoryMessages), defaultMonthlyLimit:Number(data.defaultMonthlyLimit), quotaTimezone:"Asia/Shanghai" }) }); toast("AI 配置已保存"); await renderAIAdmin(); } catch (error) { toast(error.message, "error"); } }); });
  document.getElementById("aiQuotaForm").addEventListener("submit", async (event) => { event.preventDefault(); const button = event.submitter || event.currentTarget.querySelector("button"); await runButtonAction(button, async () => { try { const data = Object.fromEntries(new FormData(event.currentTarget)); await api("/api/v1/admin/ai/quotas", { method:"PUT", body:JSON.stringify({ userIds:data.userIds.split(",").map((x) => x.trim()).filter(Boolean), mode:data.mode, monthlyLimit:Number(data.monthlyLimit) }) }); toast("用户算力额度已更新"); await renderAIAdmin(); } catch (error) { toast(error.message, "error"); } }); });
  document.getElementById("aiCreditGrantForm").addEventListener("submit", async (event) => { event.preventDefault(); const button = event.submitter || event.currentTarget.querySelector("button"); await runButtonAction(button, async () => { try { const data = Object.fromEntries(new FormData(event.currentTarget)); const result = await api("/api/v1/admin/ai/credits", { method:"POST", body:JSON.stringify({ userId:data.userId.trim(), points:Number(data.points), note:data.note.trim() }) }); toast(`永久余额已更新为 ${Number(result.creditBalance).toLocaleString("zh-CN")} 点`); await renderAIAdmin(); } catch (error) { toast(error.message, "error"); } }); });
  content.querySelector(".runtime-grid").insertAdjacentHTML("beforeend", `<section class="runtime-panel full"><div class="runtime-panel-header"><div><h2>用户 AI 用量</h2><p class="form-hint">免费账户每月 500 点；会员永久余额在月额度用完后消耗，会员过期时冻结。</p></div><span class="status-pill">${usageResponse.total} 位用户</span></div><div class="table-wrap"><table class="data-table ai-usage-table"><thead><tr><th>用户</th><th>周期</th><th>已用月度点</th><th>预留算力点</th><th>生效月额度</th><th>月度剩余</th><th>永久余额</th><th>额度模式</th><th>操作</th></tr></thead><tbody>${usageResponse.usage.map(aiAdminUsageRow).join("") || "<tr><td colspan=\"9\">暂无用户</td></tr>"}</tbody></table></div></section>`);
  ...
  document.querySelectorAll("[data-set-ai-quota]").forEach((button) => button.addEventListener("click", () => { const form = document.getElementById("aiQuotaForm"); form.elements.userIds.value = button.dataset.setAiQuota; form.elements.mode.focus(); }));
  const testButton = document.createElement("button"); testButton.type = "button"; testButton.className = "tonal-button"; testButton.textContent = "测试提供商"; document.querySelector("#aiConfigForm .runtime-actions").append(testButton); testButton.addEventListener("click", () => runButtonAction(testButton, async () => { try { await api("/api/v1/admin/ai/config/test", { method:"POST" }); toast("提供商连接正常"); } catch (error) { toast(error.message, "error"); } }));
}
